...
const sendMarsTemperature = (callback1, callback2) => {
  const currentTemperature = getMarsTemperature(); // Pega a temperatura atual, acessando a função.
  const messageSuccessfullySent = Math.random() <= 0.6; // Faz a condição de envio ser de 60% de precisão.
  setTimeout (() => {
    // se a msg for enviada
